...
import { detectPlatform, isValidUrl, getStandardFormats, extractVideoDetails } from '../downloader';

describe('Video Downloader Logic', () => {
  ...
  it('returns standard formats including 4K', () => {
  ...
  it('extracts video details properly', () => {
    const info = extractVideoDetails('https://www.youtube.com/watch?v=dQw4w9WgXcQ');
    expect(info.platform).toBe('youtube');
    expect(info.formats.length).toBe(5);
